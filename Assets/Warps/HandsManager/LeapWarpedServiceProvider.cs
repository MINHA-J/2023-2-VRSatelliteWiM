...
public class LeapWarpedServiceProvider : LeapProvider {
    ...
    private void TransformHand(Frame frame, ProxyNode proxyNode, MarkNode markNode, HandChirality handChirality) {
        ...
        if(handChirality == HandChirality.Left) {
            frame = frame.Transform(new LeapTransform(-leapRoot.position.ToLeap(), Quaternion.identity));
            frame = frame.Transform(new LeapTransform(Vector3.zero, deltaRotation.ToLeap(), scaleLeftHand));
            frame = frame.Transform(new LeapTransform(markPosition.ToLeap(), Quaternion.identity));
        } else {
            frame = frame.Transform(new LeapTransform(-leapRoot.position.ToLeap(), Quaternion.identity));
            frame = frame.Transform(new LeapTransform(Vector3.zero, deltaRotation.ToLeap(), scaleLeftHand));
            frame = frame.Transform(new LeapTransform(markPosition.ToLeap(), Quaternion.identity));
    ...
    private void ActualProvider_OnUpdateFrame(Frame frame) {
        ...
        DispatchUpdateFrameEvent(lastUpdateFrame);
    }
    ...
    private void ActualProvider_OnFixedFrame(Frame frame) {
        ...
        DispatchFixedFrameEvent(lastFixedFrame);
    }
