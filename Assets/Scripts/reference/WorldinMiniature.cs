...
public class WorldinMiniature : MonoBehaviour
{
    ...
    public GameObject right_index_finger;
    public GameObject duplicate;
    public GameObject rightRayIntersectionSphereMiniature;
    public GameObject rightRayIntersectionSphereWorld;
    public GameObject avatar = null;
    public GameObject avatar_duplicate = null;
    ...
    private Vector3 miniature_scale = new Vector3(0.05f, 0.05f, 0.05f);
    ...
    void Start()
    {
        ...
        center_eye = GameObject.Find("Main Camera");
        //left_index_finger = GameObject.Find("L_index_meta");
        //right_index_finger = GameObject.Find("R_index_meta");

        _xrController = left_hand.GetComponent<XRController>();

        
        // create avatar that can be moved to define the new position create_avatar_representation();
    ...
    {
        rightRayRenderer = right_hand.GetComponent<LineRenderer>();
        if (rightRayRenderer == null) rightRayRenderer = right_hand.AddComponent<LineRenderer>() as LineRenderer;
    ...
    void create_world_in_miniature()
    {
        ...
        duplicate.transform.position = left_hand.transform.position;
    ...
    void Update()
    {
        ...
        {
            // update avatar position update_avatar();
            // activates / deactivates the world in miniature visibility
            word_in_miniature_activation();
            // casts a ray and checks whether the hit was on the world in miniature or the original world ray_cast();
            // allow user to drag the selected object
            dragging();
            // allows the user to specify a new position avatar_jumping();

            duplicate.transform.position = left_hand.transform.position;
            duplicate.transform.localRotation = left_hand.transform.localRotation * Quaternion.Inverse(initial_rotation);
        }
        else
        {
    ...
    {
        if (_xrController.inputDevice.TryGetFeatureValue(CommonUsages.trigger, out float value))
        {
            ...
            if (!map_visible && value > 0.8f)
            {
                ...
                initial_rotation = left_hand.transform.localRotation;
                map_visible = true;
                last_values = 1;
            }
    ...
    {
        // mapping: grip button (middle finger)
        //if (_xrController.inputDevice.TryGetFeatureValue(CommonUsages.trigger, out float gripButton))
        { //Debug.Log("middle finger rocker: " + gripButton);
    ...
    private void SelectObject(GameObject go)
    {
        if (go.name != "LowpolyTerrain")
        {
            ...
            selected_object.transform.SetParent(right_hand.transform, true);
        }
    }
